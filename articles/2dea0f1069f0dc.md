...
> **◉目的**
> 部分-全体階層を表現するために、オブジェクトを木構造に組み立てる。(後略)

部分-全体階層という言葉がややこしいですが、部分が階層的に連なって最終的に1つの塊となる構造っていうイメージでいます。
...
後者の図を見ると木構造になっているのがわかりやすいと思います。
...
間違えてLeafに対してadd()を呼び出す実装をしたらコンパイル時にエラーになるので、安全性が得られます。
実行時にエラーが出て、そこで初めて実装が間違えていることを知る、のではなくその前に知ることができますね。
...
こんなに書かなくても、そういえばそもそもHTMLって木構造だよなぁとか、ひと通り書いてから思いました。木生えますわ🌲
...
export interface Toc {
  toHtml(): HTMLElement;
  add(item: Toc): void; //★追加
}
// Composite役
export class Node implements Toc {
  private items: Toc[];
  ... 
  add(item: Toc): void {
...
// Leaf役
export class Leaf implements Toc {
  ...
  add(item: Toc): void {
...
- [GitHub](https://github.com/HiromasaNojima/ngx-toc/tree/main/projects/ngx-toc/src/lib)
- [npm](https://www.npmjs.com/package/ngx-toc)
